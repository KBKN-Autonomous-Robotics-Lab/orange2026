...
class Selfdrivefulldetection(Node):
    ...
    def timer_callback(self):
        ...
        final_status = "Stop" if (self.current_human_state == "Stop") else "Go"
        #final_status = "Stop" if (human_status == "Stop") else "Go"


        if final_status != self.previous_human_status:
            self.get_logger().info(f'status: {final_status}')

        if final_status == "Stop" and self.previous_human_status != "Stop":
            self.stop = True
            self.send_action_request()

        elif final_status == "Go" and self.previous_human_status == "Stop":
            self.stop = False
            self.send_action_request()

        self.previous_human_status = final_status
