...
class ReflectionIntensityMap(Node):
    ...
    def __init__(self):
        ...
        self.white_line = self.create_publisher(PointCloud2, 'white_line', 10)
...
########################################
        ...
        self.MAP_RANGE = 7.0 #[m]auto nav 7m  selfdrive:12
        
        self.MAP_RANGE_GL = 7 #[m] 
        self.MAP_LIM_X_MIN = -7.0 #[m]
        self.MAP_LIM_X_MAX =  7.0 #[m]
        self.MAP_LIM_Y_MIN = -7.0 #[m]
        self.MAP_LIM_Y_MAX =  7.0 #[m]
        ...
        # DBSCANで使う白線候補抽出パラメータ
        self.line_intensity_threshold = 22.0
        self.dbscan_eps = 0.08
        self.dbscan_min_samples = 5
        self.dbscan_min_cluster_size = 10
    ...
    def filter_ground_points_dbscan(self, ground_points):
        if ground_points is None or ground_points.shape[1] == 0:
            return np.empty((4, 0), dtype=np.float32)

        self.get_logger().info(f"ground points total: {ground_points.shape[1]}")

        intensity_mask = ground_points[3, :] >= self.line_intensity_threshold
        candidate_points = ground_points[:, intensity_mask]

        self.get_logger().info(f"after intensity threshold: {candidate_points.shape[1]}")

        if candidate_points.shape[1] == 0:
            return np.empty((4, 0), dtype=np.float32)

        xy = candidate_points[:2, :].T

        db = DBSCAN(eps=self.dbscan_eps, min_samples=self.dbscan_min_samples)
        labels = db.fit_predict(xy)

        valid_mask = np.zeros(candidate_points.shape[1], dtype=bool)
        ...
        filtered_points = candidate_points[:, valid_mask]

        self.get_logger().info(f"after DBSCAN: {filtered_points.shape[1]}")
    ...
    def reflect_map(self, msg):
        ...
        # DBSCAN用に intensity 付きの ekf座標系点群を作る
        ekf_ground_points_with_intensity = np.vstack((
            ekf_ground_set_x,
            ekf_ground_set_y,
            ekf_ground_set_z,
            ground_reflect_conv
        ...
        # 強反射候補 + DBSCANでノイズ除去
        filtered_line_points = self.filter_ground_points_dbscan(ekf_ground_points_with_intensity)

        if filtered_line_points.shape[1] == 0:
            self.get_logger().warn("DBSCAN後の点群が0です。processをスキップします")
            return

        # 白線候補専用マップを作る
        if filtered_line_points.shape[1] > 0:
            filtered_line_reflect = filtered_line_points[3,:]

            line_map_data_gl_set = grid_map_set(
                filtered_line_points[1,:],
                filtered_line_points[0,:],
                filtered_line_reflect,
                ekf_position,
                self.ground_pixel,
                self.MAP_RANGE_GL
            )
        else:
            line_map_data_gl_set = np.zeros_like(map_data_gl_set, dtype=np.uint8)
        ...
        self.process(line_map_data_gl_set, ekf_position_x, ekf_position_y, ekf_theta_z, t_stamp)
        ...
        if self.MAKE_GL_MAP_FLAG == 1:
            ...
            #self.process(map_data_gl_set, ekf_position_x, ekf_position_y, ekf_theta_z, t_stamp)
            pass
    ...
    def process(self, map_data_set, position_x, position_y, theta_z, t_stamp):
        try:
            ...
            self.update_angle()
            #reflect_map_local = self.rotate_image(image, -self.theta_z +90)
           ...
           #right_line,left_line = self.generate_lines (right_point, left_point, interval = 0.1, extend = 0.5, offset_distance=2.2, direction="right")## line theta koushin
            ...
            opened = cv2.morphologyEx(binary_image, cv2.MORPH_OPEN, kernel_open)
            oc_image = cv2.morphologyEx(opened, cv2.MORPH_CLOSE, kernel_close) 
            ...
            if white_point is not None:
                # ランダム間引き
                random_mask = np.random.rand(white_point.shape[0]) > 0.4  #60%nokoru
                white_point = white_point[random_mask, :]

                if white_point.shape[0] > 0:
                    timestamp = time.time()
                    timestamps = np.full((white_point.shape[0], 1), timestamp)

                    points_with_time = np.hstack((white_point, timestamps))

                    if self.white_buff.size:
                        self.white_buff = np.hstack((self.white_buff, points_with_time.T))
                    else:
            ...
            # white_buff publish
            timestamp = time.time()
            timestamps = np.full((white_point.shape[0], 1), timestamp)  # N x 1

            points_with_time = np.hstack((white_point, timestamps))

            if self.white_buff.size:
                self.white_buff = np.hstack((self.white_buff, points_with_time.T))  # shape 5 x N
            else:
                self.white_buff = points_with_time.T
    ...
    def update_angle(self, max_delta = 15.0, max_history = 5): # max_delta = 15.0
        ...
        if self.left_flag == 1 and self.left_flag == 1:
            self.angle = self.angle + self.left_angle
        else:
    ...
    def binarize_image(self, image):
        _, binary_image = cv2.threshold(image, self.binary_threshold, 255, cv2.THRESH_BINARY)# 255
        return binary_image
